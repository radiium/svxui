<script lang="ts">
    import H2 from '$lib/components/markdown/elements/h2.svelte';
...
{#if items.length}
    {#if category === 'components' || category === 'attachments'}
        <H2 id="api-reference">API Reference</H2>
    {:else if category === 'builders' || category === 'utilities'}
        <H2 id="type-definition">Type Definition</H2>
    {/if}

    {#each items as item (item.name)}
        {#snippet heading()}
            <H3 id={stringToId(item.name)}>
                {item.typeName}
            </H3>
        ...
        {:else if isUtility(item) || isBuilder(item)}
            {#if item.classDef}
                <H3 id={stringToId(item.className)}>
                    {item.className}
                </H3>
                <CodePreview>
                    {#snippet code()}
            ...
            {#if item.typeDef}
                <H3 id={stringToId(item.typeName)}>
                    {item.typeName}
                </H3>
                <CodePreview>
                    {#snippet code()}
