...
export class SvelteAnalyzer {
    ...
    private findPropsDeclaration(script: AST.Script): any {
        ...
        for (const node of body) {
            // Look for: let { ... } = $props()
            if (node.type === 'VariableDeclaration' && node.kind === 'let' && node.declarations.length > 0) {
     ...
     */
    private extractPropsFromDeclaration(declaration: any, propsMap: Map<string, SveltePropInfo>): void {
    ...
    private extractDefaultValue(node: any): string | undefined {
        ...
        switch (node.type) {
            ...
            case 'CallExpression':
                ...
                if (node.callee.type === 'Identifier' && node.callee.name === '$bindable') {
                    return undefined;
                }
                return undefined;

            case 'TemplateLiteral':
                // Template literals as string
