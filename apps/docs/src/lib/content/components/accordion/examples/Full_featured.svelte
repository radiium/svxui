<script lang="ts">
    ...
    import { Accordion, Button, Flexbox, Panel, Text } from 'svxui';
...
<Flexbox direction="column" gap="6">
    ...
    <Accordion bind:value={selection} multiple orientation="vertical">
        {#snippet children(accordion)}
            <Flexbox direction="column" gap="3" {...accordion.rootAttrs} style="width: 300px;">
                {#each items as data (data.id)}
                    <!-- Item -->
                    {@const item = accordion.getItem(data.id, { id: data.id, disabled: data.disabled })}
